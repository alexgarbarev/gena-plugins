module OpenApiParser
  module Specification
    class Endpoint
      ...
      def initialize(path, method, raw, specification)
        @path = path
        @method = method
        @raw = raw
      ...
      def request_path_properties

        # path parameters are all 'parameters' except headers
        parameters = @raw['parameters'].reject { |element| element['in'] == 'header' } if @raw['parameters']

        #TODO: Implement $ref support for parameters
        parameters = parameters.reject { |element| element['$ref'] != nil } if parameters

        unless parameters
          return []
        end
        properties = []
        parameters.each do |content|
          schema = content['schema'].dup
          schema['description'] = content['description']
          schema['summary'] = content['summary']
          schema['example'] = content['example']

          properties << OpenApiParser::Specification::ModelProperty.property_from_schema(content['name'], schema, content['required'], content['name'])
        end
        properties
      end
      ...
      def request_body
        ...
        if @raw.at_key('requestBody.$ref')
          #if reference to `requestBodies` component
          body = @specification.schema_at_ref(@raw['requestBody']['$ref'])
        else
      ...
      def request_body_properties
        ...
        unless self.request_body
          return [], nil
        end

        # unless schema
        #   puts "Can't find request schema for request '#{request.raw['operationId']}'".red
        #   exit(-1)
        # end

        serialization, body = self.request_body['content'].first

        request_body_schema = body['schema']

        return OpenApiParser::Specification::ModelProperty.from_request_schema(request_body_schema, @specification), serialization
      end

      def response_property
        @raw['responses'].each do |key, value|
          is_first_success = (key.to_s[0] == '2')
          if is_first_success
            return OpenApiParser::Specification::ModelProperty.from_response_schema(value, @specification)
          end
        end
        return nil
      end
